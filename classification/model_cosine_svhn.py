...
class SVHN(nn.Module):
    def __init__(self, output_dim = 9,cosine_sim = False, baseline = False, eval_flag = False):
        ...
        self.features = nn.Sequential(
             ...
             nn.MaxPool2d(kernel_size = 2, stride = 2),
             #nn.Dropout(0.25),
             nn.Conv2d(in_channels = 128, out_channels = 256, kernel_size = 3,  padding=(1, 1), bias = True),
             nn.ReLU(inplace = True),
             nn.Conv2d(in_channels = 256, out_channels = 256, kernel_size = 3,  padding=(1, 1), bias = True),
             nn.ReLU(inplace = True),
             nn.MaxPool2d(kernel_size = 2, stride = 2),
             nn.Flatten()
        )
        ...
        self.classifier = nn.Sequential(
             nn.Linear(256*4*4,1024, bias = True),     ## 17
